...
import { Calendar, AlertTriangle, CheckCircle, Clock, Edit, ArrowLeft, Beaker, FileText, Users, TrendingUp } from "lucide-react";
...
export async function generateMetadata({ params }: FormulaDetailsPageProps): Promise<Metadata> {
    const formula = mockFormulae.find((f) => f.id === params.id);
    ...
    return {
        ...
        description: formula.description || `View details for ${formula.name}, a ${formula.status} formula with ${formula.totalConcentration}% concentration.`,
        openGraph: {
...
export default function FormulaDetailsPage({ params }: FormulaDetailsPageProps) {
    ...
    const ingredients = mockFormulaIngredients
        .filter((fi) => fi.formulaId === formula.id)
        .map((fi) => {
    ...
    const totalCost = ingredients.reduce((sum, ing) => sum + (ing.ingredient?.cost || 0) * (ing.percentage / 100), 0);
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">Version {formula.version}</p>
                    </div>
                </div>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                <div>
                                    ...
                                    <p className="text-lg font-semibold text-gold">{formula.totalConcentration}%</p>
                                </div>
                                <div>
                                    <span className="font-medium">Batch Size</span>
                                    <p className="text-lg font-semibold">{formula.batchSize}g</p>
                                </div>
                                <div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                {ingredients.map((ing) => (
                                    <div key={ing.id} className="flex items-center justify-between p-4 border rounded-lg">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <div className="text-sm text-muted-foreground">
                                                ${((ing.ingredient?.cost || 0) * (ing.percentage / 100)).toFixed(2)}
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <CardContent className="space-y-4">
                            ...
                            {formula.lastComplianceCheck && (
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        {formula.lastComplianceCheck.toLocaleDateString()}
                                    </p>
                                </div>
                            )}
