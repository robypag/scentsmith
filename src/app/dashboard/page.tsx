      ...
      <div className="relative space-y-4 p-8 rounded-2xl bg-gradient-to-br from-white via-yellow-50 to-amber-50 dark:from-gray-900 dark:via-yellow-900/20 dark:to-amber-900/20 border border-yellow-200 dark:border-yellow-700 shadow-xl shadow-yellow-500/10 overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl leading-relaxed">
            Welcome to your premium fragrance analytics platform. Discover insights, trends, and elevate your perfume expertise.
          </p>
          <div className="flex items-center space-x-4 mt-6">
            <Button variant="luxury" className="shadow-lg shadow-yellow-500/20 hover:shadow-xl hover:shadow-yellow-500/30">
          ...
          <LuxuryCard key={stat.id} className="group hover:scale-[1.02] transition-all duration-300">
            ...
            <CardContent>
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {stat.change && (
                  <Badge variant="secondary" className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 border-yellow-200 dark:border-yellow-700 mr-2">
                    {stat.change}
                  </Badge>
                )}
        ...
        <PremiumCard className="group">
          <CardHeader>
            <div className="flex items-center space-x-3">
              ...
              <div>
                <CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">AI Fragrance Analysis</CardTitle>
                <p className="text-sm text-gray-600 dark:text-gray-400">Advanced scent profiling</p>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Leverage cutting-edge AI to analyze fragrance compositions, predict market trends, and discover unique scent combinations.
            </p>
        ...
        <PremiumCard className="group">
          <CardHeader>
            <div className="flex items-center space-x-3">
              ...
              <div>
                <CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">Market Intelligence</CardTitle>
                <p className="text-sm text-gray-600 dark:text-gray-400">Industry insights & trends</p>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Stay ahead with real-time market data, consumer preferences, and emerging fragrance trends in the luxury perfume industry.
            </p>
            <Button variant="luxury-outline" size="sm" className="w-full">
              View Insights
            </Button>
        ...
        <PremiumCard className="group">
          <CardHeader>
            <div className="flex items-center space-x-3">
              ...
              <div>
                <CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">Premium Collections</CardTitle>
                <p className="text-sm text-gray-600 dark:text-gray-400">Curated luxury fragrances</p>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Access exclusive collections from renowned perfumers and discover rare, limited-edition fragrances.
            </p>
            <Button variant="luxury-outline" size="sm" className="w-full">
              Browse Collections
            </Button>
      ...
      <div className="space-y-4">
        ...
        <Card className="border-yellow-200 dark:border-yellow-700 bg-gradient-to-r from-white via-yellow-50/20 to-white dark:from-gray-900 dark:via-yellow-900/10 dark:to-gray-900">
          <CardContent className="p-6">
            <div className="space-y-4">
              {[
                { action: "Analyzed new fragrance", item: "Tom Ford Black Orchid", time: "2 hours ago", type: "analysis" },
                { action: "Added to collection", item: "Creed Aventus", time: "5 hours ago", type: "collection" },
                { action: "Market trend alert", item: "Woody fragrances rising", time: "1 day ago", type: "trend" },
                { action: "AI recommendation", item: "Similar scents discovered", time: "2 days ago", type: "ai" }
              ].map((activity, index) => (
                <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-white/50 dark:bg-gray-800/50 border border-yellow-100 dark:border-yellow-800 hover:border-yellow-200 dark:hover:border-yellow-700 transition-colors">
                  <div className="flex items-center space-x-3">
                    <div className={`h-2 w-2 rounded-full ${
                      activity.type === 'analysis' ? 'bg-yellow-500' :
                      activity.type === 'collection' ? 'bg-rose-500' :
                      activity.type === 'trend' ? 'bg-orange-600' : 'bg-gray-400'
                    }`} />
                    <div>
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{activity.action}</p>
                      <p className="text-xs text-yellow-600 dark:text-yellow-400">{activity.item}</p>
                    </div>
                  </div>
