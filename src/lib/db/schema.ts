import { pgTable, text, timestamp, uuid, integer, vector, decimal, boolean, jsonb } from "drizzle-orm/pg-core";
...
export const formulaIngredients = pgTable("formula_ingredients", {
  id: uuid("id").defaultRandom().primaryKey(),
  formulaId: uuid("formula_id").references(() => formulae.id).notNull(),
...
export const formulaTests = pgTable("formula_tests", {
  ...
  testedBy: uuid("tested_by").references(() => users.id).notNull(),
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
